...
class PomdpRunner:
    ...
    def run(self, algo, T, **kwargs):
        ...
        # log.info('Trajectory:')
        # log.info(self.trajectory)
        grid_array = np.array(grid3)
        # 获取行数和列数
        rows, cols = grid_array.shape
        log.info('Coverage rate:{}'.format(len(set(self.trajectory))/(rows*cols)))
        log.info('Trajectory:{}'.format(self.trajectory))
        log.info('{} games played. Toal reward = {}. Budget left = {}'.format(i + 1, total_rewards,budget))
        # print(grid3)
        return pomdp
