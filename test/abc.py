...
def generate_depth_map(shape):
    x = np.linspace(-100,100, shape[1])
    y = np.linspace(-100, 100, shape[0])
    X, Y = np.meshgrid(x, y)
    print(X)
    # 生成深度图，这里用一个简单的正弦函数作为例子
    depth_map = np.sin(np.sqrt(X**2 + Y**2) / 5)
    
    # 调整深度图的范围，使得深色更为明显
    depth_map = 1 - (1 - depth_map)**2
    return depth_map
def main(config):
    ...
    goal = config.target
    depth_map = generate_depth_map((100, 100))
    ...
    while True:
        ...
        ocean_color = plt.cm.Blues(depth_map[int(x[1] + 20), int(x[0] + 20)])
        plt.fill_between([0, 15], 0, 15, color=ocean_color)
